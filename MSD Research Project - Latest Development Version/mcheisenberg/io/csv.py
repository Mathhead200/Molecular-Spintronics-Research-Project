...
class CSV:
	def __init__(self, sim: Simulation, progress_bar: bool|str=None):
		self.sim = sim

		if progress_bar is not None and progress_bar is not False:
			if progress_bar is True:  progress_bar = "Writing CSV"
			self.progress_bar = tqdm(total=1 + max(len(sim.nodes), len(sim.history)), desc=progress_bar)  # header: 1 row
		else:
			self.progress_bar = None
	
	def open(self, out: str=None, dir: Path|str=".", prefix: str=None) -> None:
		if out is None:
			out = unique_path(dir, prefix, suffix=".csv")
		self.out = out
		self.file = open(self.out, "w", encoding="utf-8")
	...
	def write_header(self, params: Mapping[str, Any]=None):
		...
		if self.progress_bar is not None:  self.progress_bar.update(1)
	
	def write_data(self, ss: Snapshot):
		sim = self.sim
		line = StrJoiner()
		line += f'{ss.t},,'
		m = ss.m.value
		...
		for r in chain(sim.regions, sim.eregions):
			...
			line += f'{u},'
		line += ',,'
		# TODO: how to do this with only final snapshot??
		try:
			n = next(node_iter)
			if isinstance(n, Sequence):
...
def csv(sim: Simulation, out: str=None, dir: Path|str=".", prefix: str=None, params: Mapping[str, Any]=None, progress_bar: bool|str=None) -> str:
	if progress_bar is not None and progress_bar is not False:
		if progress_bar is True:  progress_bar = "Writing CSV"
		progress_bar = tqdm(total=1 + max(len(sim.nodes), len(sim.history)), desc=progress_bar)  # header: 1 row
