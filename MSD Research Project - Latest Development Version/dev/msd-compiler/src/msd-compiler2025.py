...
class Model:
	...
	def compile(self, out_path: str):
		...
		src += "\t\t\tvsubpd w, w, ymm6   ; sub by 1 -> [-1, 1); w = [u_1, v_1, u_2, v_2]\n"
		...
		src += "\t\t\tvpox s, s, s             ; [0, 0, 0, 0]\n"
		src += "\t\t\tvblendpd z, z, s, 1010b  ; [z_1, 0, z_2, 0]\n"
		src += "\t\tENDM\n"
		if "F" in self.allKeys:
			src += "\t\tmov r10, 2  ; counter from 2 to 0: # of vectors needed\n"
			src += "\t\tMARSAGIA_START_2:  ; (r10 == 2) need 2 more vectors, s' and f'\n"
			src += "\t\t\t_gen_ws ymm0, ymm3, ymm6, ymm10\n"
			src += "\t\t\tvcmppd ymm2, ymm3, ymm6, 11h  ; ymm3 < ymm6\n"
			src += "\t\t\tvmovmskpd rax, ymm2           ; rax = 0...bbaa where a = (s_1 < 1.0) and b = (s_2 < 1.0)\n"
			src += "\t\t\tF2:\ttest rax, 0100b           ; test bit (b)\n"
			src += "\t\t\t\tjz F1                     ; !b -> s_2 >= 1.0\n"
			src += "\t\t\t\t_calc_xyz ymm0, ymm2, ymm3, ymm6\n"
			src += "\t\t\t\tvperm2f128 ymm1, ymm0, ymm2, 31h  ; [x_2, y_2, z_2, 0] = f'\n"
			src += "\t\t\t\tdec r10  ; one vector down. one to go\n"
			src += "\t\t\tF1:\ttest rax, 0001b           ; test bit (a)\n"
			src += "\t\t\t\tjnz VF                     ; a -> s_1 < 1.0\n"
			src += "\t\t\t\tcmp r10, 1                ; need another vector\n"
			src += "\t\t\t\tje MARSAGLIA_START_1\n"
			src += "\t\t\t\tjmp MARSAGLIA_START_2\n"
			src += "\t\t\tVF:\tcmp r10, 1\n"
			src += "\t\t\t\tje VS\n"
			src += "\t\t\t\t_calc_xyz ymm0, ymm2, ymm3, ymm6\n"
			src += "\t\t\t\tvperm2f128 ymm1, ymm0, ymm2, 20h  ; [x_1, y_1, z_1, 0] = f'\n"
			src += "\t\t\t\tjmp MARSAGLIA_START_1\n"
			src += "\t\t\tVS:\tvperm2f128 ymm0, ymm0, ymm2, 20h  ; [x_1, y_1, z_1, 0] = s'\n"
			src += "\t\t\t\tjmp MARSALIA_END\n"
			src += "\t\tMARSAGIA_START_1:  ; (r10 == 1) need 1 more vector, s'\n"
			src += "\t\t\t_gen_ws ymm0, ymm3, ymm6, ymm10\n"
			src += "\t\t\tvcmppd ymm2, ymm3, ymm6, 11h  ; ymm3 < ymm6\n"
			src += "\t\t\tvmovmskpd rax, ymm2           ; rax = 0...bbaa where a = (s_1 < 1.0) and b = (s_2 < 1.0)\n"
			src += "\t\t\tS1:\ttest rax, 0001b           ; test bit (a)\n"
			src += "\t\t\t\tjz S2                     ; !a -> s_1 >= 1.0\n"
			src += "\t\t\t\t_calc_xyz ymm0, ymm2, ymm1, ymm6\n"
			...
			src += "\t\tMARSAGIA_START:\n"
			src += "\t\t\t_gen_ws ymm0, ymm1, ymm6, ymm10\n"
			src += "\t\t\tvcmppd ymm2, ymm1, ymm6, 11h  ; ymm1 < ymm6\n"
			src += "\t\t\tvmovmskpd rax, ymm2           ; rax = 0...bbaa where a = (s_1 < 1.0) and b = (s_2 < 1.0)\n"
			src += "\t\t\tS1:\ttest rax, 0001b           ; test bit (a)\n"
			src += "\t\t\t\tjz S2                     ; !a -> s_1 >= 1.0\n"
			src += "\t\t\t\t_calc_xyz ymm0, ymm2, ymm1, ymm6\n"
