...
class Model:
	...
	def localNodeParameterKeys(self) -> set[str]:
		"""
	...
	def compile(self, out_path: str):
		...
		keys = self.localNodeParameterKeys()  # set
		src += ".data\n"
		...
		if any(p in keys for p in {"S", "F", "kT", "Je0"}):
		...
		if any(p in keys for p in {"S", "F", "kT", "Je0"}):
			if "S" in keys:    src += f"OFFSETOF_S    EQU 32*({offset32}) + 8*(0)\n"
			if "F" in keys:    src += f"OFFSETOF_F    EQU 32*({offset32}) + 8*(1)\n"
			if "kT" in keys:   src += f"OFFSETOF_kT   EQU 32*({offset32}) + 8*(2)\n"
			if "Je0" in keys:  src += f"OFFSETOF_Je0  EQU 32*({offset32}) + 8*(3)\n"
			offset32 += 1
		...
		for i, node in enumerate(self.nodes):
			...
			if any(p in keys for p in {"S", "F", "kT", "Je0"}):
				...
				src += f", \t{S}, {F}, {kT}, {Je0}"
			src += f"  ; nodes[{i}]: id=\"{self.nodeId(node)}\"\n"
		src += "NODES ENDS\n\n"
